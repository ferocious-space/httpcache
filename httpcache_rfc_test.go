// Adapted from github.com/gregjones/httpcache (MIT, © 2012 Greg Jones).
package httpcache
...
func resetTest() {
	s.transport.Cache = newTestCache()
	clock = &realClock{}
}
...
func TestDontStorePartialRangeInCache(t *testing.T) {
	...
	}
	{
		...
		// Deviation: this fork's XFromCache header holds the cached
		// response's Date value, not upstream's literal "1" (see the
		// XFromCache doc comment in httpcache.go).
		if resp.Header.Get(XFromCache) == "" {
			t.Errorf("XFromCache header is blank; want a cache hit")
		}
	}
	{
		req, err := http.NewRequest("GET", s.server.URL+"/range", nil)
...
// Ported unchanged from upstream. A response is written to the cache only
// once the caller has read its body to EOF, so a body that is closed unread
// leaves nothing cached — storing a partial body and later serving it as a
// complete response would be worse than not caching at all.
...
// Ported unchanged from upstream. A storable response is no longer drained to
// build its cache entry; cachingReadCloser stores it as the caller reads, so
// an unbounded body neither hangs the call nor buffers before the caller sees
// anything, and a caller may read part of it and stop.
...
func TestGetOnlyIfCachedHit(t *testing.T) {
	...
	}
	{
		...
		// XFromCache doc comment in httpcache.go).
		if resp.Header.Get(XFromCache) == "" {
			t.Fatalf("XFromCache header is blank; want a cache hit")
		}
		if resp.StatusCode != http.StatusOK {
...
func TestClientTimeout(t *testing.T) {
	resetTest()
	client := &http.Client{
		Transport: NewTransport(newTestCache()),
		Timeout:   time.Second,
	}
	started := time.Now()
